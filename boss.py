...
def spawn_boss(self, screen_num):
    ...
    spawn_pos = (curr_screen.size[0] * (0.5 - curr_screen.boss_width / 2),
                 curr_screen.size[1])
    finish_pos = (curr_screen.size[0] * (0.5 - curr_screen.boss_width / 2),
                  0)
    ...
    boss_anim = kivy.animation.Animation(pos=finish_pos,
                                         duration=curr_screen.boss_intro_duration)
    boss_anim.bind(on_progress=partial(self.check_boss_collision, time_stamp, screen_num))
    boss_anim.bind(on_complete=partial(self.start_boss_animation, time_stamp, screen_num))
    boss_anim.start(boss)


def start_boss_animation(self, time_stamp, screen_num, *args):
    boss = args[1]
    curr_screen = self.root.screens[screen_num]
    new_pos = (random.uniform(0.0, 1 - boss.size_hint[0]) * curr_screen.size[0],
               random.uniform(0.0, 1 - boss.size_hint[1]) * curr_screen.size[1])
    boss_anim = kivy.animation.Animation(pos=new_pos,
                                         duration=curr_screen.boss_intro_duration)
    boss_anim.bind(on_progress=partial(self.check_boss_collision, time_stamp, screen_num))
    boss_anim.bind(on_complete=partial(self.boss_animation_completed, time_stamp, screen_num))
    boss_anim.start(boss)


def boss_animation_completed(self, time_stamp, screen_num, *args):
    self.start_boss_animation(time_stamp, screen_num, *args)
...
def check_boss_collision(self, time_stamp, screen_num, *args):
    ...
    gap_x = curr_screen.width * curr_screen.boss_width / 2
